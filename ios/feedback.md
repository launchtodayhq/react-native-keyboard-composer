...
11. No gesture-based keyboard dismiss on scroll
    (Resolved) Keyboard can be dismissed by dragging the scroll view (native `keyboardDismissMode = .interactive` + velocity-based dismiss).
...
14. Multiple keyboard observers
    ...
    (7) Magic numbers: still present (we tuned some, but they’re not centralized yet).
    (9) Streaming token animation: not part of this library right now.
    (10) Velocity matching to keyboard: not implemented.
    (11) Dismiss keyboard by scroll gesture: not implemented.
    (12) Thinking state: not implemented.
    (13) Wrapper/composer coupling (finding views in hierarchy): still there.
    (14) Multiple keyboard observers: still there (scroll handler + wrapper + composer).
